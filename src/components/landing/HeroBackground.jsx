...
function GlassGlobe({ shouldReduceMotion }) {
  ...
  useFrame((state, delta) => {
    ...
    const rotationSpeed = shouldReduceMotion ? 0 : 0.05;
  ...
  return (
    <group ref={globeGroupRef} scale={[1.3, 1.3, 1.3]} position={[0, 0, 0]}>
      ...
      {/* 5. Rim Lighting & Depth - ONLY GOLD, NO PURPLE */}
    ...
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-[#0D0D0F]">
      ...
      >
        ...
        >
          ...
          {/* Persistent Gold Spark Particles */}
          {!shouldReduceMotion && (
            <Sparkles 
              count={60}
              scale={12}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#0D0D0F] via-transparent to-transparent opacity-80 pointer-events-none" />
      </motion.div>
    </div>
  );
}
