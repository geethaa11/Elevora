...
import { Mail, Lock, Eye, EyeOff, ArrowLeft, ArrowRight, User, Briefcase } from 'lucide-react';
          ...
          {['DISCOVER', 'VALIDATE', 'CONNECT', 'BUILD', 'ELEVATE'].map((step, index) => (
            <div key={step} className="flex flex-col items-start">
              <div className="flex items-center gap-4">
                <span className={`text-[10px] font-bold tracking-widest ${index === 0 ? 'text-primary' : 'text-neutral-600'}`}>
                  0{index + 1}
                </span>
                {index === 0 && <div className="h-[1px] w-8 bg-primary" />}
              </div>
              <span className={`text-[9px] tracking-widest uppercase font-medium ${index === 0 ? 'text-neutral-50' : 'text-neutral-500'}`}>
                {step}
              </span>
            </div>
          ))}
        ...
        >
          <Card className="relative p-8 md:p-10 bg-[#1A1A1D]/80 backdrop-blur-xl border-[#2C2C34] shadow-[0_0_50px_rgba(184,134,11,0.05)] rounded-2xl overflow-hidden group">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-neutral-300 ml-1">Name</label>
                <div className="relative group/input">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 group-focus-within/input:text-primary transition-colors" size={18} />
                  <Input 
                    type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-neutral-300 ml-1">Role</label>
                <div className="relative group/input">
                  <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 group-focus-within/input:text-primary transition-colors" size={18} />
                  <select
                    value={role}
                    onChange={e => setRole(e.target.value)}
                    className="flex w-full rounded-md text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 pl-10 bg-[#111111] border-[#2C2C34] focus-visible:ring-primary/50 focus-visible:border-primary shadow-inner h-12 text-neutral-50 border outline-none appearance-none"
                  >
